...
function test() {
    ...
    function oneTest(warmup) {
        ...
        var distinct_as_numbers = {};

        for (var i = 0; i < numiterations; i++) {
            var lookup = geoip.lookup(ip_for_test());
            var as_number = lookup==undefined?'UNDEFINED':lookup.asname;
            if (distinct_as_numbers[as_number] == undefined) {
                distinct_as_numbers[as_number] = 1;
            }else {
                distinct_as_numbers[as_number] = distinct_as_numbers[as_number]+1;
            }
        }

        var finish = new Date().getTime();
        var keys = [];
        for(var k in distinct_as_numbers) keys.push(k);
        console.log("total distinct as numbers " + keys.length);
